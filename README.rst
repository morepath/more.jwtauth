...
* ``refresh_nonce_handler``: Dotted path to callback function, which receives
    ...
    refreshing. When None no nonce will be created or validated for refreshing.
...
just before, if you set the ``verify_expiration_on_refresh`` setting to True,
...
If the token gets compromised you can just change the nonce by storing
a new UUID in the user endity and the existing tokens will not be refreshed
anymore.
